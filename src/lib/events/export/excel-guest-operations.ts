  ...
  for (const group of tableGroups) {
    for (const seat of group.seats) {
      ...
      row.values = [
        ...
        companion && companion.names.length > 0 ? companion.names.join(", ") : (guest && guest.plusOnes > 0 ? `+${guest.plusOnes}` : "—"),
  ...
  if (unassignedGuests.length > 0) {
    ...
    for (const uGuest of unassignedGuests) {
      ...
      row.values = [
        ...
        "—",
        uGuest.name,
        companion.names.length > 0 ? companion.names.join(", ") : (uGuest.plusOnes > 0 ? `+${uGuest.plusOnes}` : "—"),
