...
export async function createDuplicateResolution(
  ...
  const payload = {
    ...
    duplicate_name_normalized: duplicateName ? normalizeGuestName(duplicateName) : null,
  ...
  if (error) throw new Error(`[duplicate-resolutions] create: ${error.message}`);
...
export async function findResolutionByFingerprint(
  ...
  const { data, error } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  if (error) throw new Error(`[duplicate-resolutions] findByFingerprint: ${error.message}`);
...
export async function findResolutionByEmailPhoneName(
  ...
  if (input.normalizedEmail) filters.push(`duplicate_email.eq.${input.normalizedEmail}`);
  if (input.normalizedPhone && input.normalizedPhone.length >= 8) {
    filters.push(`duplicate_phone.eq.${input.normalizedPhone}`);
  }
  if (input.normalizedName) filters.push(`duplicate_name_normalized.eq.${input.normalizedName}`);
  ...
  const { data, error } = await supabase
    ...
    .order("resolved_at", { ascending: false })
    .limit(20);
  if (error) throw new Error(`[duplicate-resolutions] findByEmailPhoneName: ${error.message}`);
...
export async function findResolutionForImportRow(
  ...
  const { data, error } = await supabase
    ...
    .order("resolved_at", { ascending: false })
    .limit(100);
  if (error) throw new Error(`[duplicate-resolutions] findForImportRow: ${error.message}`);
...
export async function getDuplicateResolutionById(
  ...
  const { data, error } = await supabase
    ...
    .eq("id", resolutionId)
    .maybeSingle();
  if (error) throw new Error(`[duplicate-resolutions] getById: ${error.message}`);
  return asTableRow<"guest_duplicate_resolutions">(data);
}
...
export async function updateDuplicateResolution(
  ...
  const supabase = createAdminClient();
  const now = new Date().toISOString();
  const payload: Record<string, unknown> = { updated_at: now, resolved_at: now };
  if (patch.resolutionStatus !== undefined) payload.resolution_status = patch.resolutionStatus;
  if (patch.primaryGuestId !== undefined) payload.primary_guest_id = patch.primaryGuestId;
  if (patch.notes !== undefined) payload.notes = toNullableText(patch.notes);
  if (patch.resolvedBy !== undefined) payload.resolved_by = toNullableText(patch.resolvedBy);
  ...
  const { data, error } = await supabase
    ...
    .select("*")
    .single();
  if (error) throw new Error(`[duplicate-resolutions] update: ${error.message}`);
  const row = asTableRow<"guest_duplicate_resolutions">(data);
...
export async function markResolutionIgnored(
  ...
  const { data, error } = await supabase
    ...
    .select("*")
    .single();
  if (error) throw new Error(`[duplicate-resolutions] markIgnored: ${error.message}`);
  const row = asTableRow<"guest_duplicate_resolutions">(data);
...
export async function listEventDuplicateResolutions(
  ...
  const { data, error } = await supabase
    ...
    .eq("event_id", eventId)
    .order("resolved_at", { ascending: false });
  if (error) throw new Error(`[duplicate-resolutions] list: ${error.message}`);
