...
export async function handleClientEventChecklistRequest(
  ...
  try {
    ...
    return { status: 500, body: { ok: false, error: "unavailable", message: "Não foi possível carregar a checklist." } };
  }
}
...
export async function loadClientEventChecklistModuleData(
  ...
  const authClient = envCheck.ok
    ? await createClientEventReadAuthClient<ClientEventChecklistAuthClient>()
    : null;
