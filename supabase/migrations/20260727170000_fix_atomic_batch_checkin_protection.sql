-- Progressive Fix: Atomic Guest Import Batch Removal Check-in & RSVP Protection Alignment
-- Created via: 20260727170000_fix_atomic_batch_checkin_protection.sql
--
-- Replaces remove_guest_import_batch_atomic to align checkin and RSVP protection
-- with the canonical application schema (guests.status enum + public.checkins table).
...
BEGIN
  ...
  -- 3. Check protected guests (RSVP confirmed/declined, seat assigned, checkin status/table, invite sent)
  SELECT pg_catalog.count(*) INTO v_protected_count
  FROM public.guests g
  WHERE g.id = ANY(v_guest_ids)
    AND (
      g.status IN ('confirmed', 'declined', 'checked_in')
      OR g.seat_id IS NOT NULL
      OR g.invite_sent_at IS NOT NULL
      OR EXISTS (
        SELECT 1 FROM public.checkins c WHERE c.guest_id = g.id AND c.event_id = p_event_id
      )
    );
