...
- **Requisitos de Prova:** Planta de arquitectura homologada ou medição in situ com fita laser, aplicando a **Directriz Editorial HAXR** (recomendação interna de 1,80 m entre centros de mesas de 10 lugares para circulação de gala e vestidos volumosos; normas legais de segurança contra incêndios e lotação regulamentar: `REQUIRES_REGULATORY_SOURCE`);
- **Estados Possíveis:** `true` | `false` | `A_CONFIRMAR`.
...
- **Requisitos de Prova:** Realização de vistoria técnica presencial com preenchimento da Checklist HAXR ou coordenação executiva de evento no terreno devidamente atestada por `OWNER_CONFIRMED_PHYSICAL_VISIT` ou `DOCUMENTARY_EVIDENCE` (associações históricas isoladas a eventos não constituem prova de visita);
- **Estados Possíveis:** `true` | `false` | `A_CONFIRMAR`.
...
| **3** | **Segurança Energética (EDM)** | Gerador de emergência a diesel com capacidade nominal declarada para cargas essenciais (iluminação, som e climatização) e comutador automático de transferência (ATS); carece de ensaio de carga real (`REQUIRES_TECHNICAL_VERIFICATION`). | Teste de carga real com simulação de corte de rede pública EDM. |
| **4** | **Climatização & Conforto Térmico**| Sistema de ar condicionado (chiller ou splits distribuídos) capaz de manter conforto térmico com sala em plena lotação no Verão de Maputo. | Medição termométrica em três pontos da sala durante evento/teste. |
| **5** | **Acústica & Isolamento** | Tempo de reverberação adequado para fala inteligível e música ao vivo; cumprimento das normas municipais de ruído nocturno (`REQUIRES_REGULATORY_SOURCE`). | Teste auditivo com sistema de som e verificação de licença de ruído. |
| **6** | **Cálculo de Lotação Científica** | Área útil de refeição adequada por convidado em mesas redondas; corredores centrais e vias de evacuação desimpedidos segundo regulamentação dos bombeiros (`REQUIRES_REGULATORY_SOURCE`). | Verificação com medição a laser e planta de distribuição. |
| **7** | **Apoio a Catering & Higiene** | Copa dedicada a brigadas externas com bancadas em inox, pontos de água corrente potável, escoamento com caixa de gordura e acesso directo para cargas. | Inspecção visual da copa/cozinha e vias de acesso de serviço. |
| **8** | **Sanitários Nobres & Acessibilidade**| Cabines sanitárias dimensionadas com limpeza permanente, ventilação e acessibilidade adaptada declarada (`REQUIRES_TECHNICAL_VERIFICATION`). | Contagem de cabines e teste funcional de água e drenagem. |
| **9** | **Camarim Privado dos Noivos** | Sala privativa exclusiva com ar condicionado, espelho de corpo inteiro, iluminação adequada, poltronas e sanitário privativo. | Inspecção do camarim e fechadura de segurança. |
| **10**| **Estacionamento & Segurança** | Parque de estacionamento fechado ou com vigilância contratada, com acessos transitáveis no período das chuvas. | Inspecção do piso e perímetro de segurança. |
...
## 6. Rastreabilidade de Evidência ao Nível do Campo (Field-Level Source Traceability)

Para evitar que uma fonte genérica atribua indevidamente veracidade a múltiplos atributos técnicos, a governação de dados da HAXR Signature exige que cada facto de alto valor seja indexado de forma individualizada:

| Atributo / Facto de Alto Valor | Tipo de Fonte Mínimo Requerido | Referência Documental / Rastreabilidade | Regra de Atribuição |
| :--- | :--- | :--- | :--- |
| **Capacidade Máxima Sentada** | `OFFICIAL_SOURCE` ou `DOCUMENTARY_EVIDENCE` | Ficha técnica oficial do hotel/espaço ou planta cotada | Proibido combinar salas ou relvados sem descrição explícita na fonte |
| **Endereço Geográfico & Bairro** | `VERIFIED_EXTERNAL_SOURCE` ou `OFFICIAL_SOURCE` | Cadastro predial, alvará ou demarcação georreferenciada | Múltiplos marcadores divergentes forçam `A_CONFIRMAR` |
| **Contacto Telefónico Directo** | `OFFICIAL_SOURCE` | Teste de chamada com atendimento comprovado | Proibido assumir telefone a partir de redes comunitárias |
| **Gerador de Emergência (kVA)** | `DOCUMENTARY_EVIDENCE` ou `OFFICIAL_SOURCE` | Placa de características do gerador ou contrato de manutenção | Sem teste de comutação automática permanece `A_CONFIRMAR` |
| **Acessibilidade para Cadeira de Rodas**| `DOCUMENTARY_EVIDENCE` ou `VISITED_BY_HAXR` | Relatório de inspecção de rampas e sanitários adaptados | Sem vistoria in situ classifica-se como declarada pela gestão |
| **Estacionamento Privado (Vagas)** | `OFFICIAL_SOURCE` ou `VISITED_BY_HAXR` | Contagem física de baías ou declaração corporativa | Números estimados sem demarcação registam-se como `A_CONFIRMAR` |
| **Preçário / Modelo Tarifário** | `OFFICIAL_SOURCE` | Proposta comercial oficial ou preçário em vigor | Vedações orais de intermediários não são aceites |
