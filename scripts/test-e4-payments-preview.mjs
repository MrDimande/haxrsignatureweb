/**
 * E.4.2 preview smoke — GET /api/events/[eventId]/payments with real auth.
 */
...
async function getPayments(eventId) {
  const response = await fetch(
    `${API_BASE}/api/events/${encodeURIComponent(eventId)}/payments`,
    {
...
const own = await getPayments(staging.eventId);
const foreignEventId = await resolveForeignEventId();
const foreign = await getPayments(foreignEventId ?? FOREIGN_EVENT_ID_FALLBACK);
const missing = await getPayments("00000000-0000-4000-8000-000000009999");
const unauth = await fetch(
  `${API_BASE}/api/events/${encodeURIComponent(staging.eventId)}/payments`,
  { cache: "no-store" },
).then(async (response) => ({
...
const demo = await fetch(
  `${API_BASE}/api/events/${encodeURIComponent("jessica-samuel")}/payments`,
  { cache: "no-store" },
).then(async (response) => ({
  status: response.status,
  body: await response.json(),
...
const ownPaymentsOk =
  ...
  Array.isArray(own.body.data?.recentPayments) &&
  typeof own.body.data?.summary?.paid === "number";
...
const summarySnapshot = own.body.ok
  ? {
      estimated: own.body.data.summary.estimated,
      paid: own.body.data.summary.paid,
      pending: own.body.data.summary.pending,
      paymentCount: own.body.data.recentPayments.length,
    }
  : null;
  ...
  JSON.stringify(
    {
      ...
      demo: {
        status: demo.status,
        ok: demo.body.ok,
        paymentCount: demo.body.data?.recentPayments?.length ?? null,
      },
    },
    null,
    2,
