/**
 * E.4.5 preview smoke — GET /api/events/[eventId]/documents with real auth.
 */
...
async function getDocuments(eventId) {
  const response = await fetch(
    `${API_BASE}/api/events/${encodeURIComponent(eventId)}/documents`,
    {
...
const own = await getDocuments(staging.eventId);
const foreignEventId = await resolveForeignEventId();
const foreign = await getDocuments(foreignEventId ?? FOREIGN_EVENT_ID_FALLBACK);
const missing = await getDocuments("00000000-0000-4000-8000-000000009999");
const unauth = await fetch(
  `${API_BASE}/api/events/${encodeURIComponent(staging.eventId)}/documents`,
  { cache: "no-store" },
).then(async (response) => ({
...
const demo = await fetch(
  `${API_BASE}/api/events/${encodeURIComponent("jessica-samuel")}/documents`,
  { cache: "no-store" },
).then(async (response) => ({
  status: response.status,
  body: await response.json(),
...
const ownDocumentsOk =
  own.status === 200 &&
  own.body.ok === true &&
  own.body.data?.context?.eventId === staging.eventId &&
  Array.isArray(own.body.data?.documents) &&
...
const summarySnapshot = own.body.ok
  ? {
      ...
      proposals: own.body.data.summary.proposals,
      contracts: own.body.data.summary.contracts,
      receipts: own.body.data.summary.receipts,
      pendingValidation: own.body.data.summary.pendingValidation,
      documentCount: own.body.data.documents.length,
    }
  : null;
  ...
  JSON.stringify(
    {
      ...
      demo: {
        status: demo.status,
        ok: demo.body.ok,
        documentCount: demo.body.data?.documents?.length ?? null,
      },
    },
    null,
    2,
