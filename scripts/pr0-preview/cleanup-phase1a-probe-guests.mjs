...
const SMOKE = process.env.HAXR_SMOKE_EVENT || "64b791b4-49c4-4b55-a8a0-99424c3d7167";
const c = new pg.Client(buildPgClientConfig());
await c.connect();
try {
  const before = await c.query(`select count(*)::int as guests from public.guests`);
  const found = await c.query(
    `select id, event_id, email, guest_source, status, created_at
     from public.guests
     where email ilike 'pr0-nobypass-probe%'
    ...
    JSON.stringify(
      {
        ...
        matches: found.rows.map((r) => ({
          ...
          status: r.status,
          emailDomain: String(r.email).split("@")[1] || null,
