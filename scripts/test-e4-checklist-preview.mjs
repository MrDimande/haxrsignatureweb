/**
 * E.4.4 preview smoke — GET /api/events/[eventId]/checklist with real auth.
 */
...
async function getChecklist(eventId) {
  const response = await fetch(
    `${API_BASE}/api/events/${encodeURIComponent(eventId)}/checklist`,
    {
...
const own = await getChecklist(staging.eventId);
const foreignEventId = await resolveForeignEventId();
const foreign = await getChecklist(foreignEventId ?? FOREIGN_EVENT_ID_FALLBACK);
const missing = await getChecklist("00000000-0000-4000-8000-000000009999");
const unauth = await fetch(
  `${API_BASE}/api/events/${encodeURIComponent(staging.eventId)}/checklist`,
  { cache: "no-store" },
).then(async (response) => ({
...
const demo = await fetch(
  `${API_BASE}/api/events/${encodeURIComponent("jessica-samuel")}/checklist`,
  { cache: "no-store" },
).then(async (response) => ({
  status: response.status,
  body: await response.json(),
...
const ownChecklistOk =
  own.status === 200 &&
  own.body.ok === true &&
  own.body.data?.context?.eventId === staging.eventId &&
  Array.isArray(own.body.data?.tasks) &&
...
const summarySnapshot = own.body.ok
  ? {
      ...
      completed: own.body.data.summary.completed,
      overdue: own.body.data.summary.overdue,
      progress: own.body.data.summary.progress,
      taskCount: own.body.data.tasks.length,
    }
  : null;
  ...
  JSON.stringify(
    {
      ...
      demo: {
        status: demo.status,
        ok: demo.body.ok,
        taskCount: demo.body.data?.tasks?.length ?? null,
      },
    },
    null,
    2,
