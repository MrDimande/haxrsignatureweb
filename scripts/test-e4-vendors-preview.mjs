/**
 * E.4.3 preview smoke — GET /api/events/[eventId]/vendors with real auth.
 */
...
async function getVendors(eventId) {
  const response = await fetch(
    `${API_BASE}/api/events/${encodeURIComponent(eventId)}/vendors`,
    {
...
const own = await getVendors(staging.eventId);
const foreignEventId = await resolveForeignEventId();
const foreign = await getVendors(foreignEventId ?? FOREIGN_EVENT_ID_FALLBACK);
const missing = await getVendors("00000000-0000-4000-8000-000000009999");
const unauth = await fetch(
  `${API_BASE}/api/events/${encodeURIComponent(staging.eventId)}/vendors`,
  { cache: "no-store" },
).then(async (response) => ({
...
const demo = await fetch(
  `${API_BASE}/api/events/${encodeURIComponent("jessica-samuel")}/vendors`,
  { cache: "no-store" },
).then(async (response) => ({
  status: response.status,
  body: await response.json(),
...
const ownVendorsOk =
  ...
  Array.isArray(own.body.data?.vendors) &&
  typeof own.body.data?.summary?.active === "number";
...
const summarySnapshot = own.body.ok
  ? {
      active: own.body.data.summary.active,
      inReview: own.body.data.summary.inReview,
      signedContracts: own.body.data.summary.signedContracts,
      vendorCount: own.body.data.vendors.length,
    }
  : null;
  ...
  JSON.stringify(
    {
      ...
      demo: {
        status: demo.status,
        ok: demo.body.ok,
        vendorCount: demo.body.data?.vendors?.length ?? null,
      },
    },
    null,
    2,
